...
import { ArrowRight, ArrowUpRight, CalendarDays, Check, MapPin, Search, Users, SlidersHorizontal, Building2, Trees, Hotel, Home, PartyPopper } from 'lucide-react';
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (user?.role === 'customer') api<string[]>('/favourites/ids').then(ids => setSavedIds(new Set(ids))).catch(() => {});
    else setSavedIds(new Set());
  }, [user]);
  async function toggleSave(venue: Venue) {
    ...
    setSavingId(venue.id); setError('');
    try {
      await api(`/favourites/${venue.id}`, { method: saved ? 'DELETE' : 'POST' });
      setSavedIds(current => { const next = new Set(current); saved ? next.delete(venue.id) : next.add(venue.id); return next; });
  ...
  return <main><section className="hero hero-video"><video className="hero-video-media" autoPlay muted loop playsInline preload="metadata" aria-hidden="true"><source src="/celebratecg-hero.mp4" type="video/mp4" /></video><div className="hero-video-shade" aria-hidden="true" /><div className="hero-copy"><span className="eyebrow"><span className="tiny-star"><CelebrationMark size={21} /></span> CHHATTISGARH’S CELEBRATION BOOKING PLATFORM</span><h1>Plan. Book.<br /><em>Celebrate.</em></h1><p>Find beautiful stays, celebration spaces and event services across Chhattisgarh. A place for every occasion.</p><div className="hero-note"><span className="note-icon"><Check size={14} /></span> Launching {site.launch}</div><div className="hero-actions"><a className="button button-primary" href={site.signup}>Create an account <ArrowUpRight size={16} /></a><Link className="text-button" href="/signup?role=vendor">List your property <ArrowUpRight size={15} /></Link></div></div><div className="hero-video-caption"><span className="live-dot" /> Your people. Your kind of place.</div></section>
